...
import { getAccountsDashboard, getAllVouchers } from '../../api/accounts';
...
const AccountsVouchersCard = ({ navigate }) => {
    ...
    <div className={s.card}>
      <div className={s.voucherCardHeader}>
        ...
        <div className={s.voucherFilters}>
          <input
            id="accounts-dashboard-voucher-search"
            type="text"
            value={searchQuery}
          ...
          />
          <select
            id="accounts-dashboard-status-filter"
            value={statusFilter}
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem', color: '#6b7280', fontWeight: 600 }}>
            From
            <input
              id="accounts-dashboard-from-date-filter"
              type="date"
          ...
          <label style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', fontSize: '0.8rem', color: '#6b7280', fontWeight: 600 }}>
            To
            <input
              id="accounts-dashboard-to-date-filter"
              type="date"
              value={dateToFilter}
              ...
              <tbody>
                {filtered.map((voucher) => (
                  <tr
                    ...
                    onClick={() => navigate(`/accounts/vouchers/${voucher.id}`)}
                  >
    ...
    <div className={s.page}>
      ...
      <div className={s.container}>
        <div className={s.pageHeader}>
          <h1 className={s.pageTitle}>Accounts Dashboard</h1>
        </div>
          ...
          <>
            <div className={s.statsGrid}>
              ...
              <div className={`${s.statCard} ${s.statCardAmount}`}>
                <div className={s.statValue}>INR {Number(stats.totalApprovedExpenseAmount).toLocaleString('en-IN')}</div>
                <div className={s.statLabel}>Total Approved Expense</div>
              </div>
            </div>

            <AccountsVouchersCard navigate={navigate} />
          </>
        ) : null}
      </div>
    </div>
